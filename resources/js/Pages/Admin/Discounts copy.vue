...
/* TABLE */
// Define column sets for each discount type
const baseColumns = [
...
const columnsByType = {
    'Potongan Harga Produk': [
        ...baseColumns,
        { accessorKey: 'amount', header: 'Jumlah Diskon' },
    ],
    'Potongan Harga Grosir': [
        ...baseColumns,
        { accessorKey: 'threshold', header: 'Minimum Jumlah Produk' },
        { accessorKey: 'amount', header: 'Jumlah Diskon' },
        { accessorKey: 'amount_type', header: 'Tipe Diskon' },
    ],
    'Potongan Harga Belanja': [
        ...baseColumns,
        { accessorKey: 'threshold', header: 'Minimum Belanja' },
        { accessorKey: 'amount', header: 'Jumlah Diskon' },
        { accessorKey: 'amount_type', header: 'Tipe Diskon' },
    ],
    'Beli X Gratis Y': [
        ...baseColumns,
        { accessorKey: 'buy_quantity', header: 'Jumlah Beli' },
        { accessorKey: 'get_quantity', header: 'Jumlah Gratis' },
    ],
};
...
const allColumns = [
    ...
    { accessorKey: 'amount', header: 'Jumlah Diskon' },
    { accessorKey: 'amount_type', header: 'Tipe Diskon' },
    { accessorKey: 'threshold', header: 'Batas' },
    { accessorKey: 'buy_quantity', header: 'Jumlah Beli' },
    { accessorKey: 'get_quantity', header: 'Jumlah Gratis' },
];

// Computed property to determine which columns to use
const columns = computed(() => {
    if (!data.value || data.value.length === 0) {
        return allColumns;
    }

    const discountTypes = new Set(data.value.map(discount => discount.type));

    if (discountTypes.size === 1) {
        const type = discountTypes.values().next().value;
        return columnsByType[type] || allColumns;
    } else {
        return allColumns;
    }
});
...
const groupedData = computed(() => {
    ...
    data.value.forEach(item => {
        switch (item.type) {
            case 'Potongan Harga Produk':
                groups['Potongan Harga Produk'].push(item);
                break;
            case 'Potongan Harga Belanja':
                groups['Potongan Harga Belanja'].push(item);
                break;
            case 'Potongan Harga Grosir':
                groups['Potongan Harga Grosir'].push(item);
                break;
            case 'Beli X Gratis Y':
                groups['Beli X Gratis Y'].push(item);
                break;
        }
    });
...
<template>
    ...
    <AdminLayout>
        ...
        <div>
            <!-- Table  -->
            <div>
                <!-- Render a table for each discount type -->
                <div v-for="(discounts, type) in groupedData" :key="type" class="mb-10">
                    <h2 class="text-xl font-semibold mb-4">{{ type }}</h2>
                    <div class="rounded-md border">
                        <Table>
                            <TableHeaderWrapper :columns="columnsByType[type]" :sorting="sorting" @sort="sortBy" />

                            <TableBody>
                                <TableRow v-for="(row, index) in discounts" :key="row.id">
                                    <TableCell>
                                        {{ index + 1 }}
                                    </TableCell>

                                    <TableCell v-for="column in columnsByType[type]" :key="column.accessorKey">
                                        {{ row[column.accessorKey] }}
                                    </TableCell>

                                    <TableCell>
                                        <div class="flex gap-2">
                                            <Button @click="() => openEditModal(row)">Ubah</Button>
                                            <Button @click="() => openDeleteModal(row)">Hapus</Button>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            </TableBody>
            ...
            <DialogWrapper v-model:open="isAddModalOpen" title="Tambah Diskon" customClass="md:w-[50rem]"
                ...
                <form @submit.prevent="onSubmit" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div class="flex gap-4 item-center">
                        <div v-if="form.values.type == 'product' || form.values.type == 'buy_x_get_y'" class="w-full">
                            <FormField v-slot="{ field }" name="category_id">
                                <FormItem>
                                    <FormLabel>Pilih Produk</FormLabel>
                                    <Multiselect v-model="selectedProducts" :options="productOptions"
                        ...
                        <div v-if="form.values.type == 'order' || form.values.type == 'quantity'" class="w-full">
                            <FormInput v-if="form.values.type == 'order'" name="threshold" label="Minimum Belanja"
                                type="number" />
                            <FormInput v-if="form.values.type == 'quantity'" name="threshold"
                                label="Minimum Jumlah Produk" type="number" />
                        </div>

                        <div v-if="form.values.type != 'buy_x_get_y'" class="flex gap-1 item-center w-full">
                    ...
                    <div v-if="form.values.type == 'buy_x_get_y'" class="flex gap-4 item-center">
                        <FormInput name="buy_quantity" label="Jumlah Beli" type="number" />
                        <FormInput name="get_quantity" label="Jumlah Gratis" type="number" />
                    </div>
