...
const onSubmit = form.handleSubmit(async (values) => {
    try {
        ...
        let response = await axios.post(`/admin/restocks/${selectedRestock.value.id}?_method=PUT`, values);
...
const deleteRestock = async () => {
    if (selectedRestock.value) {
        try {
            const response = await axios.post(`/admin/restocks/${selectedRestock.value.id}?_method=DELETE`);
            if (response.data.status === 'error') {
                return Toast.fire({
                    icon: "error",
                    title: response.data.message,
    ...
    <AdminLayout>
        ...
        <div class="flex flex-col md:flex-row justify-between">
            ...
            <div class="flex items-center py-4 w-full md:w-72">
                <Input placeholder="Cari Restock..." v-model="globalFilter" class="w-full max-w-full md:max-w-sm"
        ...
        <div>
            ...
            <!-- Edit Modal -->
            <DialogWrapper v-model:open="isEditModalOpen" title="Ubah Data Restock" desc="Ubah data restock">
                <form @submit="onSubmit" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <DialogFooter>
                        <Button type="submit" :class="{ 'bg-slate-500': isLoading }" :disabled="isLoading">
                            {{ isLoading ? 'Mohon tunggu ...' : 'Tambah Restock' }}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogWrapper>
            ...
            <!-- Delete Modal -->
            <DialogWrapper v-model:open="isDeleteModalOpen" title="Hapus Restock" desc="Hapus restock">
                <DialogFooter>
                    <Button @click="isDeleteModalOpen = false" variant="outline">Batal</Button>
                    <Button @click="deleteRestock" variant="destructive">Hapus</Button>
                </DialogFooter>
            </DialogWrapper>
        </div>
    </AdminLayout>
