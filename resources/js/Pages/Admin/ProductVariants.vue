...
const onSubmit = async (values) => {
    try {
        ...
        let response = await axios.post(`/admin/product-variants/${selectedProduct.value.id}?_method=PUT`, form.values);
...
const deleteProduct = async () => {
    if (selectedProduct.value) {
        try {
            ...
            const response = await axios.post(`/admin/product-variants/${selectedProduct.value.id}?_method=DELETE`);
            if (response.data.status === 'error') {
                return Toast.fire({
                    icon: "error",
                    title: response.data.message,
...
<template>
    ...
    <Head title="Daftar Variasi Produk" />

    <AdminLayout>
        <!-- Judul Halaman -->
        <h1 class="text-2xl font-semibold text-gray-900">Daftar Variasi Produk</h1>
        ...
        <div class="flex flex-col md:flex-row justify-between">
            <div class="flex gap-2">
                ...
                <a href="/admin/product-variants/export-excel">
