...
class ProductController extends Controller
{
    ...
    public function getProduct(Request $request)
    {
        ...
        // Use cache to store and retrieve product data
        $cacheKey = 'product_' . $request->sku;
        $product = Cache::remember($cacheKey, now()->addHours(24), function () use ($request) {
            return Product::where('sku', $request->sku)->with('productVariants')->first();
        });
        ...
        $transformedProducts = array_merge(
            $product->toArray(),
            [
    ...
    public function getProductByName(Request $request)
    {
        ...
        // Remove caching for name-based searches
        $products = ProductVariant::where('name', 'like', '%' . $request->name . '%')->with('productVariants')->get();
